...
[^2]: Not all algorithms can take advantage of sparse representations of data. In such cases, a sparse matrix must be converted to a more conventional format before proceeding.
...
The `r pkg(magrittr)` pipe operator (`%>%`) is a tool for chaining together a sequence of R functions.[^3] To demonstrate, consider the following commands that sort a data frame and then retain the first 10 rows:

[^3]: In R 4.1.0, a native pipe operator `|>` was introduced as well. In this book, we use the `r pkg(magrittr)` pipe since users on older versions of R will not have the new native pipe.
...
[^5]: Chapter 10 of @wickham2016 has more details on tibbles.
...
3.  Convert the date field to the R date format using the `mdy()` function from the `r pkg(lubridate)` package. We also convert the ridership numbers to thousands. Both of these computations are executed using the `dplyr::mutate()` function.
...
```{r tidyverse-process, message = FALSE}
